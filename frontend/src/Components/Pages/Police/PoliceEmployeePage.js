...
import PoliceOfficersTable from './PoliceOfficersTable'

const PoliceEmployeePage = ({ userRole, officeLocation }) => {
  ...
  const handleSearch = () => {
    ...
    axios
      .get(`http://localhost:5000/api/police/search`, {
  ...
  return (
    ...
    <style>
      ...
      <ListItemButton
        component={Link}
        to={`/${userRole}/${officeLocation}/addpoliceofficer`}
        className="custom-link-style"
      >
        Add policeofficer
      </ListItemButton>
          <br/><br/>
      <PoliceOfficersTable  userRole={userRole} officeLocation={officeLocation}  searchResults={searchResults} />
    </Container>
  )
  
}

export default PoliceEmployeePage